...
ROUTES = [
    ...
    {'path': '/ithinkthereforiam', 'title': 'I Think Therefore I Am — Prateek Gupta',
     'description': DESCRIPTION, 'type': 'website'},
    {'path': '/qr', 'title': 'QR Maker — Prateek Gupta',
     'description': DESCRIPTION, 'type': 'website'},
    {'path': '/share', 'title': 'Sealed Note — Prateek Gupta',
     'description': DESCRIPTION, 'type': 'website', 'private': True},
]
for a in ARTICLES:
...
STASH = ('\n    <script>'
         '(function(){if(location.hash.length>1){window.__sealed=location.hash.slice(1);'
...
def shell(route, noindex=False):
    ...
    html = TEMPLATE.replace('<!DOCTYPE html>' + chr(10), '<!DOCTYPE html>' + chr(10) + BANNER, 1)
    ...
    if route.get('private'):
        # no analytics on this page, and the fragment (which carries an encrypted note)
        # is taken out of the address bar before any other script can see it
        html = sub(html, r'\n\s*<script defer src="https://cloud\.umami\.is[^>]*></script>', '')
        html = sub(html, r'\n\s*<!-- Google tag \(gtag\.js\) -->.*?gtag\(\'config\'[^\n]*\n\s*</script>', '')
        html = sub(html, r'(<base href="/k/">)', r'\1' + STASH)
